...
class SiderMenu extends PureComponent {
  ...
  */
  ...
  getSubMenuOrItem=(item) => {
    if (item.children && item.children.some(child => child.name[this.props.uiLang])) {
      return (
        <SubMenu
          title={
            item.icon ? (
              <span>
                {getIcon(item.icon)}
                <span>{item.name[this.props.uiLang]}</span>
              </span>
            ) : item.name[this.props.uiLang]
            }
          key={item.path}
        >
  ...
  */
  getNavMenuItems = (menusData) => {
    ...
    return menusData
      .filter(item => item.name[this.props.uiLang] && !item.hideInMenu)
      .map((item) => {
      ...
      >
        <div className={styles.logo} key="logo">
          <Link to="/">
            ...
            <h1>{SITE_TITLE[this.props.uiLang]}</h1>
          </Link>
        </div>
        <Menu
          key="Menu"
